<template>
  <div class="bg-[#F8F6F4] min-h-screen font-sans text-[#333]" dir="rtl">
    ...
    <div v-else-if="view === 'product'" class="animate-in">
      <nav class="p-8 flex justify-start items-center max-w-7xl mx-auto">
        <button @click="view = 'home'" class="text-[#3D5A50] font-bold text-lg flex items-center gap-3 hover:gap-5 transition-all p-3 px-8 rounded-full bg-white shadow-sm border border-[#E5E0DA]">
          <span>→</span> العودة للرئيسية
        </button>
      </nav>

      <section class="max-w-7xl mx-auto pt-6 px-6 pb-24">
        <div class="flex flex-col lg:flex-row items-start gap-16 lg:gap-24">
          ...
          <div class="w-full lg:w-2/5 text-right">
            <h1 class="text-6xl font-black mb-6 text-[#333] leading-relaxed">{{ activeProductData.title }}</h1>
